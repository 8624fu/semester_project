...
def run_cv(
    ...
    for f in sorted(fold_id.unique()):
        ...
        #LZ updated to save risk scores, 07_05
        ...
        #LZ updated to save best-fold model weights and features for SHAP analysis, 07_05
        current_best = max([r["test_c_index"] for r in rows], default=0)
        if test_c_index >= current_best:
            torch.save(model.net.state_dict(),
                       Path("../results/tables/nn_cox_integrated_best_fold_weights.pt"))
            np.save("../results/tables/nn_cox_integrated_shap_input.npy", x_tr)
            n_meth = x_tr.shape[1] - len(rna.columns)
            meth_train_raw = meth.loc[tr_ids]
            missing_rate = meth_train_raw.isna().mean()
            variance = meth_train_raw.var(skipna=True)
            keep_cpgs = list(meth_train_raw.columns[
                (missing_rate <= METH_MISSING_THRESHOLD) &
                (variance >= meth_variance_threshold)
            ])[:n_meth]
            rna_names  = [f"RNA: {g}" for g in rna.columns]
            meth_names = [f"METH: {cpg}" for cpg in keep_cpgs]
            pd.Series(rna_names + meth_names).to_csv(
                "../results/tables/nn_cox_integrated_feature_names.csv",
